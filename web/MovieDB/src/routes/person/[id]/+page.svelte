<script>
  ...
  async function load() {
    ...
    const personData = await fetch(
      `https://api.themoviedb.org/3/person/${id}?api_key=799c0bd0b2baaecc6d9301fadfaea7f7&language=en-US`
    ).then(resPerson => resPerson.json());
    personPic = personData
    person = data.person
...
<section class="grid grid-cols-1 sm:grid-cols-3 gap-1 lg:grid-cols-4 gap-6 my-6">
  <div class="col-span-1 w-[35%] mx-auto sm:w-auto">
    <img src={`https://image.tmdb.org/t/p/original${personPic.profile_path}`} 
    alt="Poster" class="rounded-lg mb-8"
    >
  </div>
